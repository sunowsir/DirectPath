...
function import_ip () {
    ...
    do_import_ip  "/tmp/CN-ip-cidr.txt"
    rm -rf /tmp/CN-ip-cidr.txt
    
    wget -q https://raw.githubusercontent.com/Hackl0us/GeoIP2-CN/release/CN-ip-cidr.txt  -O /tmp/CN-ip-cidr.txt
    do_import_ip "/tmp/CN-ip-cidr.txt"
    rm -rf /tmp/CN-ip-cidr.txt 
